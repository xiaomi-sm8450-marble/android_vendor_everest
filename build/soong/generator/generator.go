...
func init() {
	android.RegisterModuleType("everest_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = everestExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = everestExpandVariables(ctx, dep_file)
	...
	cmd := everestExpandVariables(ctx, String(g.properties.Cmd))
